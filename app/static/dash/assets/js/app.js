  ...
  <div class="app">
    ...
    <main class="main"><div class="main-content">
      ...
      <div class="page" id="page-casas">
        ...
        ${mkCard('casa_kpi','Bookies — Visão Geral','<div id="casaPortfolioKPIs" style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:.75rem;margin-bottom:.75rem"></div><div class="tcard-sort"><span class="tcard-sort__lbl">Ordenar</span><div class="tcard-seg" id="casaSeg"><button data-k="pl" class="active" onclick="casaSortBy(this.dataset.k)">P/L</button><button data-k="roi" onclick="casaSortBy(this.dataset.k)">ROI</button><button data-k="to" onclick="casaSortBy(this.dataset.k)">Turnover</button><button data-k="wr" onclick="casaSortBy(this.dataset.k)">Win Rate</button><button data-k="vol" onclick="casaSortBy(this.dataset.k)">Volume</button></div><button class="tcard-dir" id="casaDir" onclick="casaSortDir()">↓</button></div><div class="tcard-grid" id="casaKpiCards"></div>')}
    ...
    <div class="analise-popup-overlay" id="apEditOverlay" onclick="fecharEdicaoApostas(event)">
      <div class="analise-popup-modal" id="apEditModal" style="max-width:560px" onclick="event.stopPropagation()">
        ...
        <div style="padding:var(--sp-5) var(--sp-6);display:grid;grid-template-columns:1fr 1fr;gap:12px 14px">
          <div class="apedit-field"><label class="apedit-label" for="ap-ed-data">Data</label><div class="shcal-datewrap"><input class="apedit-inp" id="ap-ed-data" type="text" placeholder="DD/MM/AAAA" spellcheck="false" autocomplete="off"><button type="button" class="shcal-databtn" title="Abrir calendário" aria-label="Abrir calendário" onclick="apEdAbrirCalendario()"><svg width="14" height="14" viewBox="0 0 15 15" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><rect x="1.5" y="2.5" width="12" height="11" rx="2"/><path d="M1.5 6.5h12"/><path d="M4.5 1v3"/><path d="M10.5 1v3"/></svg></button></div></div>
