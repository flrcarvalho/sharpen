...
function renderParceiros(rows){
  ...
  // As duas colunas de data saem em dd/mm/aa, que ordenado como TEXTO ordena pelo dia do
  // mês. Quem manda no sort é o `data-sort` em ISO (o `sortTable` lê o dataset antes do
  // textContent); a Casa também leva o seu porque o chip de inicial injeta uma letra a
  // mais no textContent das casas sem favicon. (s300)
  const accRows=Object.values(map).sort((a,b)=>b.l-a.l).map(e=>{const roi=e.s>0?(e.l/e.s*100):0;const lc=e.l>=0?'color:var(--pos)':'color:var(--neg)';const rc=roi>=0?'color:var(--pos)':'color:var(--neg)';const sorted=e.datas.slice().sort();const iso1=sorted[0].slice(0,10),iso2=sorted[sorted.length-1].slice(0,10);const d1=iso1.split('-'),d2=iso2.split('-');const dias=Math.round((new Date(sorted[sorted.length-1])-new Date(sorted[0]))/864e5);return`<tr><td style="font-weight:700;color:var(--ink)">${esc(e.forn)}</td><td>${esc(e.conta)}</td><td data-sort="${esc(e.casa||'')}">${casaCell(e.casa)}</td><td>${e.n}</td><td>${fmtR(e.s)}</td><td style="${lc}">${fmtPL(e.l)}</td><td style="${rc}">${fmtPct(roi,1)}</td><td data-sort="${iso1}">${d1[2]}/${d1[1]}/${d1[0].slice(2)}</td><td data-sort="${iso2}">${d2[2]}/${d2[1]}/${d2[0].slice(2)}</td><td>${dias}d</td></tr>`;}).join('');
...
// exibido nesta página — a união não cria lucro fantasma.
...
function _tmBox(nome){
  ...
  // O badge segue a MESMA régua: encostado no nome ele mudava de x a cada linha (o nome é
  // que manda na largura), então vira coluna própria no bloco da direita.
  const badgeCol=`<span style="min-width:92px;text-align:right;white-space:nowrap">${badge}</span>`;
  ...
  const header=`<div onclick="tmToggle('${_tmJs(nome)}')" style="display:flex;align-items:center;gap:12px;padding:12px 14px;cursor:pointer;user-select:none">`
    +caret+`<span style="font-weight:700;color:var(--ink);font-size:14px">${esc(nome)}</span>`
    +`<span style="margin-left:auto;display:flex;align-items:center;gap:20px">${badgeCol}${vol}${tick}</span></div>`;
