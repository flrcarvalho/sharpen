  ...
  body.innerHTML=
    ...
    `<div class="analise-popup-section">`+
      ...
      `<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:.75rem">`+
        `<div class="kpi" style="${kS}"><div class="kpi-label"><span class="kpi-pipe"></span>Topo Histórico ${_mkTipAnchor('Topo Histórico','','Maior saldo que esta casa <b>já atingiu</b> no período.','<span class="lbl">marco</span>')}</div><div class="fdc-kpi__value"${_td.topo>0?' data-state="pos"':''} style="${vS}">${fmtPL(_td.topo)}</div><div class="kpi-sub" style="${sbS}">${topoSub(_td)}</div></div>`+
...
function renderSportDrill(rows){
  ...
  body.innerHTML=
    ...
    `<div class="analise-popup-section">`+
      `<div class="analise-popup-section-title">Cenário Atual <span style="font-size:9.5px;color:var(--ink-soft);text-transform:none;letter-spacing:0">(dados reais · histórico)</span></div>`+
      `<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:.75rem">`+
        `<div class="kpi" style="${kS}"><div class="kpi-label"><span class="kpi-pipe"></span>Topo Histórico ${_mkTipAnchor('Topo Histórico','','Maior saldo que este esporte <b>já atingiu</b> no período.','<span class="lbl">marco</span>')}</div><div class="fdc-kpi__value"${_td.topo>0?' data-state="pos"':''} style="${vS}">${fmtPL(_td.topo)}</div><div class="kpi-sub" style="${sbS}">${topoSub(_td)}</div></div>`+
