...
## Onde parei (fim da sessão 317)

**Filtrar por desfecho e deixar o KPI seguir junto dá um número certo pela conta e
mentiroso pela leitura.** Filtro de resultado na Base Completa (pedido do tester João
Henrique: *"ajuda bastante para conferir e corrigir valores"*), e o Feca ampliou —
*"esses filtros são BEM POBRES"*. Eram: a barra da página (período/esporte/casa/tipster)
mais seis caixas de texto, **três delas duplicando por substring** (`Vinicius` pegava
`Vinicius2`) o multiselect que já existia 20px acima. Fora isso, nenhum jeito de cortar
por resultado, faixa de stake, faixa de odd, faixa de P/L ou conta, e nada na tela
dizendo o que estava filtrado — só o contador `400 de 400`.

A decisão que ficou (do Feca, perguntado antes de escrever uma linha): **o desfecho corta
a TABELA, não a régua.** Os filtros de texto já mexiam nos KPIs e está certo — recortar
por casa é recortar carteira. O resultado é outra natureza: filtrar `W` daria **Win Rate
100%**. Então os KPIs leem `apostasKpiRows` (o recorte da tela sem o corte por desfecho),
a tabela lê `apostasFiltered`, e **a tela diz isso numa nota enquanto o filtro está
ligado** — sem ela, ler P/L positivo filtrando `L` parece defeito.

Entregue (`charts/apostas.js`, `app.js`, `components.css`):

- **Chips de resultado** `W · HW · L · HL · V · Aberta`, multi-seleção, **com a contagem
  do recorte** (é ela que responde "quantas red eu tenho aqui" antes do clique). Rótulo é
  o código canônico; o nome humano fica no `title`.
- **Faixas** de stake, odd e P/L (mín/máx). Passam pelo `parseNum`, então aceitam
  `1.250,50` e o minus U+2212 de um copiar/colar da própria tela. **Aposta aberta sai de
  qualquer faixa de P/L**: o zero que o feed traz é ausência, e a colocaria dentro de toda
  faixa que cruze o zero.
- **Conta virou multiselect** (eixo que a barra da página não tem); as três caixas de
  texto redundantes saíram, sobrando busca em Aposta e Descrição.
- **Chips de filtro ativo** com ✕ individual, cobrindo os DOIS níveis (barra da página e
  filtros locais), mais `Limpar tudo`.
- **Todas as colunas ordenáveis** (eram 4 de 10). Resultado ordena em ordem **semântica**
  — `W · HW · V · HL · L · Aberta` —, nunca a alfabética (`HL, HW, L, V, W`), que não diz
  nada para quem confere.
- `moneyStake` entrou no dash: §5.1 prevê a máscara de 2 casas para valor unitário e o
...
estruturais — **10/10**. Duas mutações escaparam na primeira rodada e as duas eram
cegueira do teste, não mutação inócua: faltava ordem natural de dígito (`conta2` ×
`conta10`) e P/L **negativo** — sem negativo, o `localeCompare` com `numeric:true`
acertava a ordem sozinho e o ramo numérico não era load-bearing. Uma terceira escapou
porque no caso combinado a faixa sozinha já dava o mesmo número (dado que não exerce a
regra). Renderizado headless contra o `servidor_demo`, com 24 mil apostas, sem erro de
console.

**Achado que fica para depois, registrado e não copiado:** `.btbl-th`, `.btbl-counter` e
`.filter-label` usam `--ink-mute` em **9px** — label em tom de metadado, abaixo do piso da
Escada de Tinta (que pede `--ink-soft` a partir de 9,5px). São anteriores à regra. O CSS
novo (`.apf-*`) não os copia e passa nos três critérios da varredura.
