...
_Atualizado: 2026-09-04 (sessão 318 — **zero não é ausência.** Bilhete de MESMO JOGO ("Criar Aposta"/SGM) traz uma odd só, a do conjunto, e as seleções vêm sem preço. O bot montava a dupla como produto das pernas: `0 × 0 = 0`. Zero passou por toda checagem de forma — a linha tinha stake, resultado e P/L — e o P/L de W (`stake × (odd − 1)`) deu **−1u num bilhete GANHO** (Só Chutes #12, Osimhen + Shomurodov @ 5.50, os dois ✅). Hoje a ausência viaja como `null`, a combinação que cobre o CUPOM INTEIRO recebe a odd do conjunto (valia só para `tipo === 'tripla'`), o post não imprime `@ 0` nem P/L sem odd e o TSV manda a coluna vazia. **A segunda metade era pior:** o `/salvar` recusou a linha (`odd inválido`), devolveu `rejeitados` e respondeu **200** — o bot ignorava o campo, marcou planilhado e publicou. A aposta **nunca existiu na planilha**, e a pista só apareceu um dia depois, como um "não tenho o id no Sharpen". Regra no `CLAUDE.md`; `sharpen-bot` `8f9f697` + `cf3f620`; 9 de 10 mutações pegas)_
