...
- **Sessão 59 (28/06/2026) — Início da migração planilha → Postgres (unificação c/ Dashboard):**
  ...
  - **Lote 14 (Pinnacle ativa, janela bagunçada 01-15/06):** resolvido via export oficial. Pinnacle|Feca sheet 317, DB 229 — janela 01-15/06 com 52 DB-only + 25 offsets de 1 dia (data liquidação vs evento). Decisão: Feca processa export MyBets_20260628.xls no app (cobre 25/05→21/06, dedup por código + Migração B); DB vira bloco perfeito p/ esse período. Eu importo sheet Pinnacle|Feca < 25/05 = 66 bets (01/01-24/05). [AGUARDANDO Feca processar o export]
  - **Progresso prod:** Próximo na ordem: Superbet + restantes (PixBet, Liderbet, SportingBet, Rei do Pitaco, Tivo). Bet365 ATIVAS (gleice/marlon/Talia) pendentes. Betnacional pendente (acima). Base do operador: CSV separado, depois. **Padrão: selecionar por (casa,parceiro), validar stake/PL+colisão, normalizar typos de parceiro ([ Eu]→[Eu]). Arquivo fonte atual: CSV(7).**
