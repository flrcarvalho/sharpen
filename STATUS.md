...
_Atualizado: 2026-06-29 (sessão 75 — LOGIN DO OPERADOR `Lava` (multiusuário, fase 1 de 2). O Feca pediu para criar o login do operador dele e subir a base dele. **Feito nesta sessão: só o LOGIN** (a base depende do CSV que o Feca ainda vai subir — fase 2). `app/auth.py`: `USUARIOS` ganhou `"Lava": os.environ.get("SENHA_LAVA_HASH", "")`, mesmo padrão fail-closed dos outros (hash bcrypt vindo SÓ da env var no Railway, sem default no código). Senha combinada: `Lavinha`. Hash bcrypt gerado e verificado localmente (`checkpw` → True): `$2b$12$9lAE.FDOlv6yD8dfJ/0dguAR2R0eGCUQd2iV9OcPZ2x0SEXiAnhza`. **PENDENTE (Feca, no Railway):** definir a env var `SENHA_LAVA_HASH` com esse hash no serviço `extrator` + redeploy → aí o login `Lava`/`Lavinha` passa a funcionar (só sob HTTPS, igual aos outros). `py_compile` OK. Backup: `Backups/sessao75-login-operador-lava/auth.py`. **FASE 2 (base do operador):** quando o Feca subir o CSV da base do `Lava`, migrar com `dono='Lava'`, mesmos padrões da migração do Feca (era-split/backfill de tipster+odd+resultado) E carimbando `criado_em` do bloco abaixo do `T_min` dele (lógica do backfill da sessão 62), senão o import sobe ao topo da lista.)_
