...
- **Sessão 59 (28/06/2026) — Início da migração planilha → Postgres (unificação c/ Dashboard):**
  ...
  - **Progresso prod:** Próximo: Bet365 ATIVAS (gleice/marlon/Talia) + restantes (Liderbet, Tivo). Pendência: Superbet "Parceiro1" no DB não visto no sheet (placeholder? aguardar Feca). (gleice/marlon/Talia) pendentes. Betnacional pendente (acima). Base do operador: CSV separado, depois. **Padrão: selecionar por (casa,parceiro), validar stake/PL+colisão, normalizar typos de parceiro ([ Eu]→[Eu]). Arquivo fonte atual: CSV(7).**
