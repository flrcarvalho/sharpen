...
- **Sessão 59 (28/06/2026) — Início da migração planilha → Postgres (unificação c/ Dashboard):**
  ...
  - **⚠️ BETNACIONAL PENDENTE (LEMBRAR O FECA):** Feca pediu p/ PULAR a Betnacional (28/06) — casa teve problema, ele vai RE-EXTRAIR tudo no sistema antes de migrar. NÃO importar/corrigir/deletar nada dela até lá. Em aberto: 1 correção de data (Modric 22→23/06) + 7 DB-only não-no-sheet (incl. duplicata "Messi [Argentina v ?]"). Lembrar proativamente.
  ...
  - **Progresso prod:** Próximo na ordem: Pinnacle, Superbet + restantes (PixBet, Liderbet, SportingBet, Rei do Pitaco, Tivo, Betboom já feito, Faz1Bet já, etc.). Bet365 ATIVAS (gleice/marlon/Talia) pendentes. Betnacional pendente (acima). Base do operador: CSV separado, depois. **Padrão: selecionar por (casa,parceiro), validar stake/PL+colisão, normalizar typos de parceiro ([ Eu]→[Eu]). Arquivo fonte atual: CSV(7).**
