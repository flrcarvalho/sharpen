...
> Próxima sessão: (1) **Validar a nova UI no deploy** (lista estilo Dashboard, paginação, modal, autocomplete) + decidir o **modelo de login de operadores** (ver §4 sessão 60). (2) **Migrar a base do OPERADOR** (CSV separado que o Feca vai subir; mesmos padrões da migração do Feca — ver §4 lote 19). (3) **Rotacionar a senha do Postgres** no Railway (a DATABASE_URL trafegou no chat). (4) Fase C do PLANO_UNIFICACAO: endpoint `/dashboard/data` + dashboard same-origin. (5) Fase A2: colunas `pl_num`/`valor_num` (carregar P/L da planilha → habilita a coluna P/L na lista). (6) Candidatos antigos: aposentar app Polymarket standalone; cadastrar Snooker em `MASTER_ESPORTES`.
