...
- **Login `ZoraEsports` ainda 401 — env var colada NÃO basta (armadilha do seed, descoberta na s238).** O Feca colou `SENHA_ZORAESPORTS_HASH` e redeployou, mas o 401 persiste **medido**. Causa: a conta entrou no código num deploy ANTERIOR à env var → o seed do boot criou a linha em `usuarios` com hash vazio, e `seed_usuarios` é `ON CONFLICT DO NOTHING` (a docstring ainda diz "quem autentica são os dicts", mas desde o Deploy B da s233 quem manda é a TABELA via cache). **Regra geral: conta nova deployada antes da env var = linha sem hash para sempre; só UPDATE na tabela conserta.** Fix pronto em `scratchpad/fix_hash_zora.py` (o classificador do Claude Code barra mexida em credencial de produção — o Feca roda com `!`). Depois do UPDATE o cache renova em ≤60s.
