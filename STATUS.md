...
## Onde parei (fim da sessão 313)

O card "Cenário Atual" parou de mentir para quem está no vermelho. `Topo
Histórico` nunca mais fica abaixo de zero (a curva começa no zero, antes da
primeira aposta) e `Drawdown Atual` deixou de dar 0 por construção em toda
carteira que mergulhou e está se recuperando. As duas funções que descrevem a
curva — `calcDrawdownReal` e `calcTopoDrawdown` — agora partem do MESMO ponto.

Feito: `peak = 0` em `calcTopoDrawdown`, o `topoSub` único nos 4 renders,
`ddAtualPct` na régua do `mddPct`, `data-state="pos"` condicional ao topo > 0, e
`tests/js/topo_drawdown.mjs` + `tests/test_topo_drawdown.py` (7 de 8 mutações
detectadas, a 8ª provada inócua).

**Decisão do Feca, anotada e não aberta:** dois desvios do padrão monetário que
são anteriores a esta sessão e ficaram no lugar. O `Drawdown Atual` em R$ 0,00
herda o vermelho do `data-state="real"` (o §5.1 pede zero neutro), e o
`Recovery Factor` negativo imprime hífen ASCII em vez do minus U+2212, porque
`fmtOdd` usa `toLocaleString` cru. Mexer nos dois muda cor e sinal em telas que
hoje estão corretas para o caso comum — é mudança separada.
