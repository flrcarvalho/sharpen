...
> **Histórico completo das sessões 216 → 14** → [`docs/HISTORICO.md`](docs/HISTORICO.md)
...
- **Três env vars de senha faltando no Railway** (s216, s218 e s220): `SENHA_LAVAPESSOAL_HASH`, `SENHA_WILLIAMOLIVEIRA_HASH` e `SENHA_VINICIUSOLIVEIRA_HASH`. O código dos três usuários está no ar; sem as variáveis o login responde "usuário ou senha inválidos" (fail-closed, hash vazio). Os hashes foram gerados e conferidos, mas **não entram no git** — se perdidos, gerar de novo com bcrypt. Confirmação depois de colar: `POST /login` devolve **200** (401 = hash ausente ou diferente).
