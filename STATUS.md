...
> **Histórico completo das sessões 309 → 14** → [`docs/HISTORICO.md`](docs/HISTORICO.md)

---

## Onde parei (fim da sessão 311)

A stake saiu da mão da IA nas casas de API. O gatilho foi um print do Feca
comparando a lista da Pinnacle com o dashboard: um bilhete com stake `400,00`
onde a casa dizia `204,00`. Era carryover de chunk no financeiro, e a odd
derivada preservava o P/L — por isso nenhum gate acusou.

Feito: `repository.corrigir_stake_tsv` (coluna 8 vinda do bloco cru, odd de W
refeita junto), ligado nos dois caminhos do stream; a linha errada corrigida no
banco por `scripts/corrigir_stake_infiel_s311.py`; `tests/test_stake_determinista.py`
com 15 casos e 6 mutações provadas; a regra do `CLAUDE.md` que dizia "o financeiro
não viaja junto" reescrita.
