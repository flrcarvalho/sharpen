...
- **Remover o badge NEW da BetNacional quando o Feca terminar a revisão** (s231): tirar a chave `novo` de `dashboard_rows` (`app/repository.py`), o pill `bet-new-pill` de `charts/apostas.js` e a classe de `components.css` (+ bumps `?v=`). Antes disso, o Feca revisa os **14 novos sem par** (bets nunca extraídas, ainda sem tipster — filtrar Betnacional e olhar as linhas com NEW e tipster vazio) e confere 2 casos que o de-para decidiu sozinho: os **dois bilhetes idênticos 60×31,19 de 23/07** (códigos distintos na API = apostas distintas, os dois ficaram) e o par **02/07 50×4 L** (descrição legada dizia "Lamine Yamal", a da API diz "Cristiano Ronaldo"; mesmo dia/stake/odd/resultado e nenhum outro candidato — tratado como leitura ruidosa do print e deletado; P/L idêntico de qualquer forma).
