...
- **`Totais do Jogo` / `Total - 2 Opções` são rótulos genéricos — a categoria segue o objeto (`MASTER_APOSTAS §1`), não o rótulo.** Em Basquete / eBasket / Vôlei o objeto é ponto → `Pontos`. Em Futebol o mesmo tipo de mercado aparece como `Gols + -` → `Gols`. ⚠️ **A casa escreve o total de gols com DUAS grafias: `Gols + -` e `Gols +/-`** (medido em 23/09 sobre 28.473 blocos: 8.956 e 291). É o mesmo mercado — as 291 são todas **ao vivo** (`Ao-Vivo - Gols +/-`), com o placar do momento prefixando a seleção (`(5-0) - Mais de 6.5,7.0`). Quem consome o rótulo precisa das duas. Nunca classificar total de jogo como `Team Props` (é a soma dos dois times, não estatística de equipe), `Games` (unidade do Tênis) nem `Handicap`.
