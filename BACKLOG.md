...
| **Selo de captura no dashboard** | `STATUS.md` (s345) | **Etapa 1 no ar**: o selo SharpenUp ao lado do nome da casa (Painel de Contas, dropdown de conta, idcard) e o campo `captura` em `GET /casas`. Falta a **Etapa 2**, já aprovada pelo Feca: ao clicar em **Processar** com texto/print numa casa que tem captura, um modal da marca (reusa `.modal-overlay`/`.modal`, nunca o `confirm()` do Chrome) avisa que a casa é lida direto pela extensão, com **Usar o SharpenUp** e **Processar mesmo assim**. Decidido: **mensagem enxuta**, sem "não avisar de novo" e sem o aviso âmbar sobre bilhete antigo. Nascer como helper reusável, para os outros `confirm()` nativos do `index.html` terem para onde migrar depois | 🟢 |
