...
> - **Onda 3** — índices em `bilhetes` `8e6e7c4` · **teto de paginação Poly** `771aea3` · **fetch consolidado** (`coletar_tudo`, ~2× mais rápido, paridade testada) `c749056`. _(resta #13 autodiagnóstico, #14 domain.py, `_portfolio`→"—")_
> - **Poly UX** — auto-sync ao entrar/voltar (throttle 5 min, pausa se aba oculta) `efbd55f`. _(resta: sync incremental por marca d'água — precisa teste de paridade na carteira real)_
...
> - **#15/#16** Solidez — tentei gate de rentabilidade + separar força-do-sinal `1923052`, mas **REVERTIDO `9394553`**: rumo ERRADO — o gate rebaixava um tipster sólido (11/12 meses positivos) a "Baixa" só por um drawdown normal, justo o que o KPI existe pra evitar. **Solidez → sessão DEDICADA de redesenho** (mede robustez do histórico, não lucro da janela; ver memória `solidez_kpi_proposito`).
>
> **Decisões do Feca (sessão 160):** #20 aplicar os 3 · #15/#16 corrigir os dois · **#5 Poly odd → decidir junto com a Poly incremental**.
> **Migalhas MORTAS (sessão 160):** #14 domain.py **fechado** (premissa obsoleta — puras já testáveis e o #11 destravou o teste de DB sem o split; mover 550 linhas do core de dinheiro = risco por ganho cosmético); #18 backtest temporal → **roteado à frente de inteligência de tipster** (`docs/PLANO_INTELIGENCIA_TIPSTER.md`); #21 golden bilhetes → **precisa prints de amostra do Feca** (regressão print→TSV roda a IA, não é teste unitário); #25 Backups → **virou convenção** no `CLAUDE.md` invariante #4 (não copiar HISTORICO/dirs inteiros; retenção ~sessões/90d). #19/#20 ✅ feitos hoje.
> **Backlog restante = só 3 sessões próprias:** (1) **Custo tipster → Postgres** (com o Jonathan); (2) **Sessão só-Poly** (incremental + #5 odd + `_portfolio`; precisa a carteira do Feca); (3) **Sessão só-Solidez** (redesenho do KPI — robustez do histórico, não lucro da janela; ver memória `solidez_kpi_proposito`).
