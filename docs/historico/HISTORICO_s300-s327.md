# HISTÓRICO — Sessões 357 → 300

_Sessao 362: 2026-09-15 (sessao 362: **a regua de custo virou UMA so por construcao, e tres frases vencidas sairam da tela de Custos.** Detalhe no [`PLANO_CUSTOS_TELA_UNICA`](../PLANO_CUSTOS_TELA_UNICA.md) ("O que a s362 ensinou"). **A s358 ja tinha decidido LANCAMENTO** (`calcCostFiltered`, `calcCasaCost` e `custos2.js` medem o que saiu do bolso no recorte; a janela de vida virou `calcContasEmOperacao`, estoque, fora do P/L) — **entao a Fatia 5 esta desbloqueada**. **Mas as duas telas ainda derivavam a resposta por caminhos diferentes:** `_c2contas` varria o CADASTRO e datava por `adquirida_em` cru; o KPI varria cadastro ∪ BILHETE e datava por `_dataPagamento`. **Medido com o JS de producao rodando contra o Postgres REAL, antes de mexer: R$ 0 de divergencia em 8 bases** (Feca 59.600 · realtrial 30.000 · Jonathan 28.400 · Jaao26 4.600 · germano 3.600 · Gabriel 540 · Diogo e arrudex 0), total e mes a mes. **E era sorte do dado:** bate porque toda conta com custo tem cadastro e `adquirida_em` anterior a 1a aposta, coisa que o backfill deduziu — uma ja estava desalinhada (`Faz1bet/ellennfreitas`, tela 09/09 x KPI 22/08) e so nao aparecia por custar R$ 0. Hoje o `_c2contas` sai do `_contaVida`+`_dataPagamento`; **total parado, e o que mudou foi a tela deixar de ESCONDER a conta so-de-bilhete que o KPI ja cobrava** (+1 linha no Feca, +1 no realtrial). **AS TRES FRASES:** a faixa `.c2-corte` afirmava que a Visao Geral media pela *janela de vida* — falso desde a s358, a tela passou semanas afirmando um corte que o numero nao fazia; a faixa `.c2-previa` dizia que so a tabela de precos gravava e mandava lancar em `Custos de Contas`/`Custos de Tipsters`, que **sairam do menu na s358**; e o cabecalho do `custos2.js` repetia as duas. **Licao: toda frase de tela que explica o comportamento de OUTRA tela tem prazo de validade.** **GATE NOVO: `tests/test_custos_regua_unica.py` + `tests/js/custos_regua_unica.mjs`, 10 de 10 mutacoes detectadas**, em `custos2.js` E `gestao.js`. **Tres mutacoes ESCAPARAM na 1a rodada, e as tres eram defeito do TESTE:** (a) ancora nao-unica (a mesma guarda existe em duas funcoes); (b) o caso do degrau usava datas que COINCIDIAM, entao a mutacao era no-op ali; (c) **comparar aba x KPI nunca pega mutacao no `_dataPagamento`** — ela move os DOIS para o mesmo mes errado, e por isso entraram assercoes ABSOLUTAS de mes. **E a mensagem do gate mentia:** `ok(f() === x, 'veio ' + f())` chama `f` duas vezes, e a 2a ja acha o `_contaVida` construido preguicosamente pela 1a — imprimia "veio 900" num caso que reprovou com 0. Dai o `eq(obtido, esperado, msg)`. **`_dataDoPreco` saiu de dentro do `_custoDaConta`:** a data que escolhe o degrau e a da COMPRA, a que decide o mes e a do PAGAMENTO, e a tela precisa exibir o mesmo degrau que gerou o numero. **O `node --check` pegou uma CRASE minha** num comentario dentro de template literal (o caso da s296) antes de ela derrubar a pagina. Gates: 144 passed nos 7 arquivos da frente; `check-tokens` e `check_docs` verdes; tela aberta no Chrome de verdade, medida DENTRO do iframe do dash, com 102 linhas e aba = KPI = R$ 29.400. `?v=` de gestao/custos2/app em 49/18/54. **O PLANO tambem estava corrompido:** a secao «Tudo» tinha perdido os trechos entre crases (`_c2range`, `_selRange`, `null`, o nome do gate) numa escrita por heredoc, e a frase ficara ilegivel. **ACHADO QUE NAO E DESTA FRENTE, e e grave: o custo do Feca nunca esteve no servidor.** Ele nao tem linha em `custo_store`, `parceiros.custo` e NULL nas 181 contas e `fornecedor_preco` esta vazio — os R$ 59.600 saem do `CUSTO_SEED` cravado no `gestao.js` mais o `localStorage` daquele navegador, e `custo_tipster`/`custo_geral` estao vazios. A causa e a trava anti-semeadura-parcial do `loadCusto`/`ctSave`. **A sessao vizinha esta consertando isso**, por pedido do Feca na mesma hora (*"nada fique armazenado localmente no usuario"*). **PROXIMO PASSO: a Fatia 5** — custo e P/L liquido por casa em Bookies, sobre a regua definitiva.)

_Sessao 360 (tela de Custos): 2026-09-15 (sessao 360: **a tela de Custos passou a DIZER quando o custo esta so no navegador, com o botao de guardar ali mesmo.** Pedido do Feca, textual: *"SIM, urgente, e ainda avisar a todos pra fazer isso, sem falar q eh uma falha e sim um upgrade"*, depois de ele perceber que o proprio custo de conta tinha sumido da tela (*"importante que nenhum usuario perca informacoes"*). **O CASO:** o custo nasceu no `localStorage`, virou coluna no Postgres na s165, e a trava anti-semeadura-parcial do `loadCusto`/`ctSave` **so sobe o dado no SAVE**. Quem preencheu antes do multiusuario e nunca mais editou ficou com tudo na maquina, **com a tela mostrando o numero certo o tempo todo** — nao havia sintoma nenhum ate o navegador ser limpo. **MEDIDO contra o Postgres de producao: 16 donos, 480 contas cadastradas, ZERO linha de custo em `custo_store`** (Feca, arrudex, sohprops, Lava, Ewanderson1, LavaPessoal entre eles). O Feca era um: sem linha em `custo_store`, `parceiros.custo` NULL nas 181 contas, `fornecedor_preco` vazio. **E o `CUSTO_SEED` mascarou isso por meses:** 11 pares cravados no `gestao.js` que valem so para o username dele entram em memoria quando o navegador esta vazio, entao a tela dele mostrava R$ 59.600 de custo que o servidor nunca teve. **Seed mascarando ausencia e a pior forma da ausencia** — ela some no dia em que o cache some, e o dono descobre pela tela zerada. **A via de recuperacao ja existia e ninguem sabia:** `/dashboard/importar-custos.html` faz exatamente isso desde a s165, mas e URL avulsa, **fora do menu de todo mundo, inclusive do Feca**. Dai a faixa: o aviso nasce na tela onde o dado mora. **A faixa e ACIONAVEL** (`.c2-guardar`, topo da tela de Custos, com botao) — aviso sem acao foi justamente o que tirou a `.c2-previa` daquele mesmo lugar na s362. **TRES DECISOES, cada uma de um jeito diferente de perder dado.** (1) **O seed nao sobe:** a deteccao le o CACHE e o `_custoHadLegacy` (medido no load, ANTES do fallback em memoria), nunca o `custoData` — subir seed oficializaria exemplo cravado no codigo como lancamento do dono. (2) **A ORDEM do envio:** tipster/geral ANTES do preco de conta, porque `salvar_custo_conta` faz upsert na MESMA linha de `custo_store`; na ordem inversa, `/custos/store` passaria a responder `existe=true` com o blob ainda vazio. (3) **BUG ENCONTRADO E CONSERTADO no caminho:** era exatamente esse o estado que fazia o `ctLoad` adotar o vazio do servidor e o `_ctMirror` **gravar o apagao por cima do cache local** — perda de dado silenciosa, com `existe` significando *a linha existe*, nao *o blob tem conteudo*. Hoje, blob vazio no servidor + lancamento no navegador mantem o local e oferece guardar. **O botao nao pode mentir:** `custoContaSubir`/`ctSubir` ESPERAM a resposta e levantam em `!r.ok` (o `_custoPush`/`_ctPush` sao dispara-e-esquece), e envio PARCIAL diz qual metade subiu, em vez de anunciar falha total. **GATE NOVO: `tests/test_custo_so_no_navegador.py` + `tests/js/custo_so_no_navegador.mjs`, 13 de 13 mutacoes detectadas**, recortando `gestao.js`, `app.js` E `custos2.js` de producao. **Tres mutacoes escaparam na 1a rodada e as tres eram defeito do TESTE:** o caso do seed nao exercia a leitura do cache (o `_custoHadLegacy` ja barrava sozinho), e nao havia caso de falha do `/custos/store`. **Duas delas eram INOCUAS de verdade e estao registradas como tal no fim do .py**, com o porque: as duas metades da guarda barram o seed cada uma por si, entao a mutacao que vale e a que troca AS DUAS (a versao ingenua que qualquer um escreveria). Mutacao inocua nao e buraco de teste, e inventar assercao para ela seria piorar o codigo para o teste caber. **MEDIDO NA TELA, e a medicao achou defeito:** o componente `.money` e uma COLUNA de planilha (`width:100%`, alinhado a direita, `money-val` com `min-width:10ch`) e dentro de uma frase ele **estourava o paragrafo em tres pedacos** — em 1366, `6 precos de conta (` numa linha, o valor jogado na borda direita e o `)` na linha de baixo. Mesmo carve-out que o `.kpi-sub` ja tinha; a faixa caiu de 126 para **88px**. Botao em `var(--bg)` sobre `--accent` (5,8:1; `#fff` daria 3,36). Medida em 1600/1366/1100/480, faixa sempre acima dos KPIs, botao sem truncar, zero erro de pagina; e **o clique foi testado no produto real**, com os POSTs saindo na ordem `/custos/store` -> `/custos/conta` e a faixa virando o recado verde sem botao. Suite 1.137 passed; `check-tokens` e `check_docs` verdes. `?v=` de gestao/custos2/app/components em 50/19/**56**/54. **E o 56 e o registro de um caso do invariante #8 numa forma mais fina que a ja documentada:** eu escrevi `app.js?v=55` e CONFERI (o grep logo depois mostrava 55); o commit saiu com **56**. Nao foi arquivo alheio levado junto, foi **edicao alheia dentro da MINHA propria linha**, entre o meu `sed` e o meu `git add`. Benigno aqui (numero pulado nao quebra cache-bust, e so avanca), mas o mecanismo e o mesmo: **com mais de uma sessao aberta, o working tree muda embaixo de voce mesmo em linha que voce acabou de conferir.** O `git show --stat` nao teria pego, nem o `git show <sha> -- <arquivo>`, porque o arquivo E meu e a linha E minha — so a comparacao entre o que eu escrevi e o que foi commitado acusa. **Quem bumpar em seguida parte de 56.** **REGRA NOVA NO `UI_REFERENCE §5.5`, trazida de dois planos diferentes:** o `.money` e uma COLUNA (`width:100%`, `justify-content:flex-end`, `money-val` com `min-width:10ch`) e **dentro de frase ele parte o paragrafo**; a saida nao e largar o componente (isso e o desvio do §5.4), e anular as tres propriedades no escopo de quem hospeda o texto — `.kpi-sub`, `.kpi__cv` e `.c2-guardar__txt` ja fazem. Foi para o `UI_REFERENCE`, e nao para o plano desta frente, porque **duas frentes cairam no mesmo buraco em telas diferentes** (a Fatia 0 e esta), e o lugar onde alguem procura antes de escrever um `.money` e o §5. **Os 3 vermelhos do `test_changelog` seguem sendo o manifest 0.7.13 sem nota, de outra frente.** **PENDENTE, e e do Feca:** (1) o **aviso a todos** sobre guardar o custo na conta, que ele ja autorizou e pediu enquadrado como upgrade, nao como falha — **ENVIADO** (grupo `Sharpen - Testers`, destino conferido por `getChat`, `message_id 4138`, e a mesma nota na home no mesmo ato, `c2b9c6d`). **A 1a redacao foi RECUSADA pelo Feca, e a critica e a licao:** *"essa mensagem e uma merda, claramente a gnt fez cagada e esta postando o atestado dela"*. Ela abria com *"passam a ficar guardados na sua conta, e nao mais so no navegador"* — frase que **so faz sentido se antes nao estavam**, entao ela publicava o defeito em vez da capacidade. A ordem que ele mandou: **comeca pelo que a pessoa precisa FAZER, e o resto do texto mostra o que a area ganhou.** **E a medicao que destravou a reescrita foi olhar o `changelog.json`: ZERO notas de custo em 18 novidades** — a leva inteira (tela unica, regua de caixa, tipster e geral no P/L Liquido, contas em operacao, tabela de preco com vigencia, fita do bruto ao liquido) nunca tinha sido contada. Com isso a faixa deixou de ser o assunto e virou **um passo de uma area nova**: quem nao tiver faixa nenhuma le a nota inteira como release e nao fica sabendo que houve problema. **Sintoma para reconhecer isto noutra nota:** se a frase precisa descrever o estado ANTERIOR para a novidade fazer sentido, ela e atestado, nao release; (2) o aviso represado da leva de CUSTO inteira, que muda o P/L Liquido de todo mundo; (3) a nota da 0.7.13; (4) remover o CODIGO das tres telas que sairam do menu na s358; (5) o `1 conta em operacao` do print dele, ainda sem reproducao. **E O `CUSTO_SEED` CONTINUA NO CODIGO, de proposito:** tira-lo agora zeraria a tela do Feca antes de ele guardar o que tem no navegador. Sai depois que o custo dele estiver no servidor.)

_Sessao 360: 2026-09-14 (sessao 360: **auditoria dos FILTROS do dashboard, aba a aba.** Nasceu de um print da aba Tipsters com o relato *"o filtro de Tipster esta mto desatualizado. Falta dezenas de nomes"*. **Eram SEIS defeitos, e nenhum deles da erro: todos produzem uma lista que PARECE inteira.** (1) **A lista de opcao congelava no primeiro paint.** `buildHTML` roda uma vez so; com cache local ele monta as opcoes com o dado CACHEADO, e quando o feed fresco chega o `loadData` repintava a view sem tocar em seletor nenhum. Numa aba aberta por dias, todo tipster, casa, esporte ou conta novo ficava invisivel em TODOS os filtros enquanto os cards, que leem `DADOS` ao vivo, seguiam mostrando ele. "Atualizar dados" tambem nao refazia. Mesma familia da invalidacao de `_filterCache` (s322): a lista muda por FORA de quem a pintou. Entrou `atualizarOpcoesFiltros` + `msRepintar`, que preserva selecao e busca digitada. (2) **A ordem nao era pt-BR.** `.sort()` puro ordena por codigo UTF-16 e joga minuscula e acento para depois do Z: medido, **63 dos 76 tipsters do Feca fora do lugar**, com `deLucca`, `eSoccer LBB`, `eSports LG` e `fullpicks` exilados atras de `Zora`. Entrou `cmpNome`, um comparador so para os cinco eixos. (3) **NENHUMA busca por nome ignorava acento**, em 10 superficies. Medido na base inteira: 59 dos 334 tipsters, 10 dos 39 esportes, 45 das 474 contas e 3 das 98 casas tem acento. Quem digitava `criquete`, `tenis`, `formula 1` ou `araujo` recebia ZERO, e **lista vazia na tela nao se distingue de "nao existe"**. Entrou `dobra()`, que existe duas vezes porque o dash e a Extracao sao DOIS documentos. (4) **O autocomplete cortava em 50 calado**: com 76 tipsters (Feca) ou 111 (realtrial), 26 nomes nao existiam para quem rolava sem digitar. Teto foi a 200 e passou a DIZER quando corta. (5) **O filtro de Fornecedor da Custos (previa) nascia do BILHETE**, numa tela de custo: 26 contas ativas cadastradas sem nenhum bilhete e 4 fornecedores so no cadastro ficavam invisiveis. Virou cadastro uniao base. (6) **Tipsters & Metodos listava so o cadastro** enquanto Custo de Tipsters lista a uniao. A causa e que `garantir_tipster` cria o perfil quando o tipster e atribuido EDITANDO um bilhete, mas **nao roda no `/salvar`** (IA, bot, imports): tipster vindo do bot dava para cobrar e nao dava para configurar. Zero casos medidos hoje, entao e blindagem. Abrir o box passou a criar a linha, porque o editor precisa de um `id` e sem ele a tela dizia "Tipster nao encontrado no cadastro". **DECISOES DO FECA, aprovadas com os numeros na mesa:** eixo **Tipster** entrou em Esportes, Bookies, Fornecedores e Metricas (os dois descrevem a APOSTA, entao ou recortam juntos ou nenhum recorta), e eixo **Operador** entrou na Custos (previa), onde a tela aplicava metade da regra. **E o eixo novo trouxe um efeito que a sessao vizinha viu antes de mim:** com Tipster em Bookies, o drill poe P/L do recorte ao lado de um custo que NAO recorta por tipster. Consertado pela regra da Caixa, sem mexer em numero nenhum: a TELA DIZ o corte. O aviso dispara para ESPORTE tambem, e aquele buraco ja existia. **GATES:** `tests/js/opcoes_filtro.mjs` + `tests/test_opcoes_filtro.py`, **11 de 11 mutacoes detectadas**, mais dois testes de FORMA (o feed fresco chama a repintura, e os seis eixos estao nela por LISTA e nao por lembranca). **Duas mutacoes ficaram REGISTRADAS em vez de virar assercao inventada:** tirar `sensitivity:'base'` do `cmpNome` e inocuo sob o ICU do node, e o descarte de nome vazio do `_tmNomes` divide a linha com o `_ctTipsters`, entao mutar cairia na funcao errada. **Dois harness quebraram com o commit da busca** (`ReferenceError: dobra is not defined`) porque eles RECORTAM a funcao de producao e o recorte ficou com a chamada sem a definicao; consertados no `d155a1c`, e os dois ganharam prova da regra nova. Suite 1.077 passed. **O QUE NAO ESTA PROVADO, e fica dito:** a criacao do perfil sob demanda nao foi exercida em ambiente nenhum. O `servidor_demo` nao implementa o POST (405) e a base dele tem cobertura total, como a de producao. O que a tela provou e que o caminho comum nao quebrou. **DUAS TELAS DA AUDITORIA MORREM no proximo passo** (decisao do Feca: Custos de Contas, Custos de Tipsters e Fornecedores & Parceiros saem do menu na Fatia 5), entao o achado de que as duas de custo nao tem barra de filtro nenhuma nao virou pendencia, e o eixo Tipster que entrou no Fornecedores sai junto com a tela. **COORDENACAO: a sessao vizinha (custos) e esta editaram os MESMOS arquivos o dia inteiro, e os dois erros que cometemos estao medidos.** (a) **O `git show --stat` NAO pega trecho alheio dentro de arquivo compartilhado** - ele lista `dash/index.html` como "um arquivo que eu editei", e esta certo, porque o hunk do outro esta DENTRO dele. Foi assim que ela levou dois bumps meus sem ver e que eu escrevi no corpo do `91f95fa` que tinha levado dois dela quando nao tinha levado nenhum. O que pega e `git show <sha> -- <arquivo>`. **O invariante #8 do `CLAUDE.md` manda conferir com o comando que nao detecta**, e trocar aquela linha ficou como proposta no BACKLOG, esperando o Feca. (b) **Bump de `?v=` so vale junto com o codigo que ele anuncia.** Dois bumps subiram no meu commit antes do codigo deles, e quem carregasse a pagina naqueles minutos guardaria o JS velho sob a chave nova, permanentemente. Bump adiantado e pior que bump nenhum. **PROXIMO PASSO:** nada aberto desta frente. A proposta do invariante #8 esta no `BACKLOG`.)


> Os blocos completos que saíram do `STATUS.md` (362, 360, 357, 356, 354 e 353; 339 e 336 → 317), a Sessão 315 e a cadeia `_Anterior_` de 324 até 300.
>
...
## Sessao 358 (movida do STATUS.md na s360)

> Saiu da janela deslizante quando o STATUS encostou no teto de 50 KB. Texto
> integral; so os caminhos relativos foram reescritos, porque de dentro de
> docs/historico/ os antigos apontariam para fora e quebrariam o gate de links.
...
## Sessao 359 (movida do STATUS.md na s358)
...
## Sessao 349 (movida do STATUS na s352)
